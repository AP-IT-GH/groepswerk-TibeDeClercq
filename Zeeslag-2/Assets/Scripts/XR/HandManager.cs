...
public class HandManager : MonoBehaviour
{
    ...
    private void CastSelectionRays()
    {
        if (handsActive)
        {
            ...
            if (Physics.Raycast(_rightIndex.transform.position, _rightIndex.transform.TransformDirection(Vector3.forward) - new Vector3(0,0.1f,0), out hit, 0.2f)) { if (validTags.Contains(hit.collider.gameObject.tag)) hits.Add(hit); }
            ...
            if (hits.Count > 0)
            {
                ...
                if (selectedCube != lastSelectedCube && lastSelectedCube != null)
                {
                    ...
                    lastSelectedCube.StopRotate();
                    selectedCube.RotateTowardsTarget();
